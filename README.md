...
基于 [XianyuAutoAgent](https://github.com/shaxiu/XianyuAutoAgent) 和 [XianYuApis](https://github.com/cv-cat/XianYuApis) 的全面升级版本，专为闲鱼平台打造的企业级AI值守解决方案。集成了消息队列处理、智能意图识别、本地AI模型支持和多项核心功能优化。

## 📋 项目概述

本项目在原有智能闲鱼客服机器人系统基础上，进行了简单升级和功能增强，现已发展为功能完整的闲鱼AI客服解决方案。

### 🙏 致谢

- 感谢 [@shaxiu](https://github.com/shaxiu) 的 [XianyuAutoAgent](https://github.com/shaxiu/XianyuAutoAgent) 项目提供的核心AI对话框架
- 感谢 [@cv-cat](https://github.com/cv-cat) 的 [XianYuApis](https://github.com/cv-cat/XianYuApis) 项目提供的闲鱼API逆向工程支持
...
1. **技术类优先识别**：参数、规格、型号、连接、对比等关键词
2. **价格类智能匹配**：便宜、价、砍价、少点等议价词汇
3. **正则表达式补充**：`\d+元`、`能少\d+`等模式匹配
4. **大模型兜底分类**：复杂语义的智能理解

### 安全过滤机制

- **违禁词检测**：微信、QQ、支付宝、银行卡、线下等平台外交易词汇
...
# Ping命令测试
python test_ping_command.py
```

### Ping命令
发送 `/ping` 可以检查系统状态，系统会回复 `pong` 确认正常运行。
